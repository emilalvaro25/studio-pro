...
const KnowledgeBaseDetailsDrawer: React.FC<{ kb: KnowledgeBase; onClose: () => void; }> = ({ kb, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-40 flex justify-end" onClick={onClose}>
            <div 
                className="w-full max-w-lg bg-surface border-l border-border shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out" 
            ...
            >
                ...
                <div className="p-6 flex-1 overflow-y-auto space-y-6">
                    <div>
                        <h3 className="font-semibold text-text mb-3">Ask KB</h3>
                        <form onSubmit={handleSearch} className="flex gap-2">
                            <input
                                ...
                                className="flex-1 bg-background border border-border rounded-lg px-3 py-1.5 focus:ring-2 focus:ring-primary focus:outline-none"
                            />
                            <button
                                type="submit"
                                disabled={isSearching || !searchQuery.trim()}
                                className="flex items-center justify-center space-x-2 bg-primary text-white font-semibold px-4 py-1.5 rounded-lg hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                    ...
                    <div className="min-h-[100px]">
                        {isSearching && (
                            <div className="flex items-center justify-center pt-4 text-subtle">
                        ...
                        {!isSearching && searchResults.length > 0 && (
                            <div className="space-y-3">
                                <h4 className="text-sm font-semibold text-subtle">Search Results:</h4>
                                <ul className="list-disc list-inside space-y-2 text-sm text-text bg-background p-3 rounded-lg border border-border">
                    ...
                    <div>
                         <h3 className="font-semibold text-text mb-3">Chunk Preview</h3>
                         <div className="space-y-4 text-xs text-subtle bg-background p-4 rounded-lg border border-border">
                            <p className="border-b border-border pb-2"><strong>Chunk 1:</strong> Turkish Airlines provides a generous baggage allowance for all its passengers. For international flights, Economy Class passengers are typically allowed two checked bags, each weighing up to 23 kg (50 lbs)...</p>
                            <p className="border-b border-border pb-2"><strong>Chunk 2:</strong> In the event of delayed or lost baggage, passengers should immediately file a Property Irregularity Report (PIR) at the Lost and Found office at the arrival airport. This report is essential for tracing the baggage...</p>
        ...
        <Fragment>
            <div className="p-6">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <button data-id="kb-import" className="flex items-center space-x-2 bg-primary text-white font-semibold px-4 py-1.5 rounded-lg hover:opacity-90 transition-opacity">
                        <Upload size={18} />
                        <span>Import</span>
                    </button>
                </div>
                <div className="bg-surface border border-border rounded-xl overflow-hidden">
                 ...
                 <div 
                    ...
                    className={`mt-8 text-center p-10 border-2 border-dashed rounded-xl transition-colors ${
                        isDragging ? 'border-primary bg-primary/10' : 'border-border hover:border-subtle/50'
                    }`}
                >
                    <UploadCloud size={32} className={`mx-auto mb-2 transition-colors ${isDragging ? 'text-primary' : 'text-subtle'}`} />
                    <p className={`font-semibold transition-colors ${isDragging ? 'text-primary' : 'text-text'}`}>
                        Drag and drop files here
                    </p>
                    <p className="text-subtle text-sm">PDF, DOCX, or text files are supported.</p>
                </div>
            </div>
