...
const KnowledgeBaseDetailsDrawer: React.FC<{ kb: KnowledgeBase; onClose: () => void; }> = ({ kb, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-40 flex justify-end" onClick={onClose}>
            <div 
                className="w-full max-w-lg bg-card border-l border-border shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out" 
                onClick={e => e.stopPropagation()}
            >
                <div className="flex justify-between items-center p-4 border-b border-border flex-shrink-0">
                    <div>
                        <h2 className="text-lg font-semibold text-text">Knowledge Base Details</h2>
                        <p className="text-sm text-muted truncate" title={kb.source}>{kb.source}</p>
                    </div>
                ...
                <div className="p-6 flex-1 overflow-y-auto space-y-6">
                    <div>
                        ...
                        <form onSubmit={handleSearch} className="flex gap-2">
                            <input
                                ...
                                className="flex-1 bg-background border border-border rounded-lg px-3 py-1.5 focus:ring-2 focus:ring-brand-teal focus:outline-none"
                            />
                            <button
                                type="submit"
                    ...
                    <div className="min-h-[100px]">
                        {isSearching && (
                            <div className="flex items-center justify-center pt-4 text-muted">
                        ...
                        {!isSearching && searchResults.length > 0 && (
                            <div className="space-y-3">
                                <h4 className="text-sm font-semibold text-muted">Search Results:</h4>
                    ...
                    <div>
                         ...
                         <div className="space-y-4 text-xs text-muted bg-background p-4 rounded-lg border border-border">
    ...
    return (
        <Fragment>
            <div className="p-6">
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-border">
                            {knowledgeBases.map(kb => (
                                <tr key={kb.id} className="hover:bg-primary/5 cursor-pointer" onClick={() => setSelectedKb(kb)}>
                                    <td className="p-4 font-semibold text-text">{kb.source}</td>
                                    <td className="p-4 text-muted">{kb.chunks}</td>
                                    ...
                                    <td className="p-4 text-muted">{kb.updated}</td>
                                    <td className="p-4">
                                        <div className="flex items-center space-x-3 text-muted">
                                            ...
                                            <button className="hover:text-text" title="More" onClick={(e) => e.stopPropagation()}><MoreVertical size={16}/></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                ...
                >
                    <UploadCloud size={32} className={`mx-auto mb-2 transition-colors ${isDragging ? 'text-brand-teal' : 'text-muted'}`} />
                    <p className={`font-semibold transition-colors ${isDragging ? 'text-brand-teal' : 'text-text'}`}>
                        Drag and drop files here
                    </p>
                    <p className="text-muted text-sm">PDF, DOCX, or text files are supported.</p>
                </div>
            </div>
